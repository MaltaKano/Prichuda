  ...
  React.useEffect(() => {
    if (router.query.error === "CredentialsSignin") {
      return setFormStatus({
        status: "error",
        message: "нераспознанное имя пользователя или пароль, пожалуйста, попробуйте еще раз",
      })
    }
    ...
    >
      ...
      <div className="grid gap-2">
        <label htmlFor="username" className="font-semibold text-text">
          {"Логин"} <span className="text-sm text-red-500">*</span>
        </label>
        <input
          id="username"
          name="username"
        ...
        />
        <p className="text-sm text-text">{"Введите ваш Логин"}</p>
      </div>
      <div className="grid gap-2">
        ...
        <p className="text-sm text-text">
          {"Введите Пароль, соответствующий вашему Логину"}
        </p>
      </div>
      <div>
        <input
          ...
          value={
            formStatus?.status === "fetching" ? "подождите..." : "войти"
          }
        />
      </div>
    </form>
